...
function redactValue(value: string, agentKey: string | undefined): string {
  ...
  return result
    ...
    .replace(/((?:gateway[_-]?key)["'\s:=]+)[^\s"',}]+/gi, "$1[REDACTED]")
    .replace(/((?:api[_-]?key|token|secret|password)["'\s:=]+)[^\s"',}]+/gi, "$1[REDACTED]");
}

export function redactDiagnostic(value: unknown, agentKey?: string): unknown {
...
export function redactText(value: string, agentKey?: string): string {
